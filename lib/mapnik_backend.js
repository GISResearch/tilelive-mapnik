...
MapnikSource.prototype._createMetatileCache = function(options) {
    var source = this;
    this._tileCache = new LockingCache(function(key, requestId) {
        var cache = this;
        ...
        var keys = source._renderMetatile({
            ...
            if (err) {
                ...
                for (var key in tiles) {
                    cache.put(key, null, tiles[key].image, tiles[key].headers, tiles[key].stats, tiles[key].requestId);
                }
            }
        });
    ...
    }); // purge immediately after callbacks
};

// Render handler for a given tile request.
MapnikSource.prototype.getTile = function(z, x, y, options, callback) {
    z = +z; x = +x; y = +y;
    ...
    this._tileCache.get(key, requestId, function(err, tile, headers, stats, requestId) {
        if (err) return callback(err);
        callback(null, tile, headers, stats, requestId);
    });
};

MapnikSource.prototype.getGrid = function(z, x, y, options, callback) {
    z = +z; x = +x; y = +y;
    if (isNaN(z) || isNaN(x) || isNaN(y)) {
        return callback(new Error('Invalid coordinates: '+z+'/'+x+'/'+y));
    }
    ...
    var requestId = options.requestId;
    this._tileCache.get(key, requestId, function(err, grid, headers, stats, requestId) {
        if (err) return callback(err);
        delete grid.solid;
        callback(null, grid, headers, stats, requestId);
    });
};
